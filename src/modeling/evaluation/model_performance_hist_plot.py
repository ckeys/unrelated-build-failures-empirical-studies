...
def performance_plot(base_path, metric='f1'):
    ...
    df_filtered = all_data[all_data['model'] == 'PULearning'].drop(columns=['model'])
    # Step 2: Create intervals for dropout values
    intervals = pd.cut(df_filtered['dropout'], bins=[0, 25, 50, 75, 100], include_lowest=True)
    # Step 3: Add intervals as a new column to the DataFrame
    df_filtered['dropout_interval'] = intervals
    # Step 4: Group by project and dropout intervals
    grouped = df_filtered.groupby(['project', 'dropout_interval'])
    # Plotting for f1
    plt.figure(figsize=(12, 6))
    sns.boxplot(x='dropout_interval', y='f1', hue='project', data=df_filtered)
    plt.title('Boxplot of F1 Scores by Dropout Intervals and Project')
    plt.xlabel('Dropout Interval')
    plt.ylabel('F1 Score')
    plt.legend(title='Project', bbox_to_anchor=(1.0, 1), loc='upper left')
    plt.show()

    # # Step 2: Group by project, dropout, and iteration, then calculate median values
    # df_grouped = df_filtered.groupby(['project', 'dropout', 'iteration']).median().reset_index()
    # # Step 3: Calculate median values for each project across dropout and iteration
    # df_result = df_grouped.groupby('project').median().reset_index()
    # # Step 4: Rename columns for clarity
    # df_result = df_result[['project', 'f1', 'recall']]
    # df_result.columns = ['project', 'f1-median', 'recall-median']
    #
    # print(df_result)
...
base_path = os.path.join('/'.join(current_file_path.split('/')[:-4]), 'data','modeling_data','training','training_with_LOOCV_performance_r1' )
